...
@section('content')
    <div class="row">
        <div class="card" style="width: 500px; height: 600px; margin-right: 20px">
            <div class="card-header">Edit Modular Group</div>
            <div class="card-body">
                <form action="{{ route('update_modular_group', 'id=' . $role->id) }}" method="post" id="modular_group">
                    @csrf
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                                <label for="">Modular Group<span style="color: red">*</span></label>
                                <input type="text" name="modular_group" value="{{ $role->name }}"
                        ...
                        @foreach (\App\Libraries\Permissions::permissions() as $key1 => $per)
                            <li class="branch" style="cursor: pointer;">
                                <input type="checkbox" name=""
                                    {{ $role->hasAnyPermission(array_keys($per)) ? 'checked' : '' }} value=""
                                    {{-- checked="checked" --}} class="check_all">{{ $key1 }}
                                <ul>
                                    @foreach ($per as $key2 => $sub_per)
                                        <li class="" style="cursor: pointer; display: none;"><input type="checkbox"
                                                ...
                                                name="module_permissions[]" value="{{ $key2 }}"
                                                {{ $role->hasPermissionTo($key2) ? 'checked' : '' }}
