...
@section('content')
        ...
        <div class="card-body">
            <div class="row">
                @if (Auth::user()->role != 'Supervisor')
                    <div class="col-md-2">
                        <select name="role" id="order_role" class="form-control form-control-sm">
                ...
                @endif
                <div class="col-md-2">
                    <select name="user" class="form-control form-control-sm" id="order_user">
                        @if (Auth::user()->role == 'Supervisor')
    ...
    {{--    Modal --}}
    {{-- Total Order Target --}}
    <div class="modal fade total_order_target" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
        aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Total Order Target (<span class="show_name"></span>)
                    </h5>
                ...
                <div class="modal-body">
                    ...
                    <table class="table" id="myTable" width="100%">
                        ...
                        <tbody class="insert_order_targets">
                        </tbody>
                    </table>
                </div>
...
@section('javascript')
    <script>
        ...
        $('#order_role').change(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    $('#order_user').html(data.order_users);
                },
        ...
        $('#filter').click(function() {
            ...
            var user_id = $('#order_user').val();
            $.ajax({
                url: "{{ route('order_target') }}",
                method: 'get',
                dataType: 'json',
                data: {
                    from_date: from_date,
                ...
                success: function(data) {
                    if (data.hasOwnProperty('data')) {
                        ...
                        output += '<td>' + data.username.name.charAt(0).toUpperCase() + data.username
                            .name.slice(1) + '</td>';
                        output += '<td>' + data.total_targets.order_target_otc + '</td>';
                        output += '<td>' + data.total_targets.order_target_mrc + '</td>';
                        output += '<td>' + data.total_otc + '</td>';
                        output += '<td>' + data.total_mrc + '</td>';
                        // output += '<td>' + parseInt(((parseInt(data.total_otc) + parseInt(data.total_mrc)) / (parseInt(data.total_targets_otc.order_target_otc) + parseInt(data.total_targets_mrc.order_target_mrc)) * 100 )) + '&nbsp;<b>%</b></td>';
                        output += '<td>' + parseInt(((parseInt(data.total_otc)) / (parseInt(data
                            .total_targets.order_target_otc)) * 100)) + '&nbsp;<b>%</b></td>';
                        output += '<td>' + parseInt(((parseInt(data.total_mrc)) / (parseInt(data
                            .total_targets.order_target_mrc)) * 100)) + '&nbsp;<b>%</b></td>';
                        output +=
                            '<td><a class="C_total_order_target" data-toggle="modal" data-user_id="' +
                            user_id + '" data-from_date="' + from_date + '" data-to_date="' + to_date +
                            '" data-target=".total_order_target"><i class="fa-solid fa-eye"style="font-size:16px;color: rgb(50, 152, 195)"></i></a></td>';
                        output += '</tr>';
                        $('.order_target').html(output);
                    }
                },
                error: function(XMLHttpRequest) {
                    console.log(XMLHttpRequest.responseJSON.message);
        ...
        $(document).on('click', '.C_total_order_target', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (i = 0; i < total; i++) {
                        output += '<tr>';
                        output += '<td>' + data.total_targets[i].order_purposal_date + '</td>';
                        output += '<td>' + data.total_targets[i].company_name + '</td>';
                        // output += '<td>'+data.total_targets[i].order_purposal_qty+'</td>';
                        // output += '<td>'+data.total_targets[i].order_purposal_sale+'</td>';
                        output += '<td>' + data.total_targets[i].order_purposal_total_amount + '</td>';
                        output += '<td>' + data.total_targets[i].order_purposal_payment_type + '</td>';
                        output += '<td>' + data.total_targets[i].order_purposal_created_at + '</td>';
                        output += '</tr>';
                    }
                    $('.insert_order_targets').html(output);
