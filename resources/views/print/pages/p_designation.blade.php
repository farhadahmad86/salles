...
@section('print_cntnt')
    @if ($count_row > 0)
        <table class="table" id="myTable">
            ...
            <tbody id="designation_table_row">
                @foreach ($datas as $index => $designation)
                    <tr>
                        <td>{{ $index + $datas->firstItem() }}</td>
                        <td>{{ $designation->designation_title }}</td>
                        <td>{{ $designation->name }}</td>
                        <td>{{ date('d-M-Y', strtotime($designation->designation_created_at)) }}</td>
                        @if (empty($type))
                            <td>
                                <div class="dropdown">
                                    ...
                                    <a href="{{ Route('edit_designation', 'id=' . $designation->designation_id) }}" class=""><i
                                            class="fa-solid fa-file-pen" style="font-size:16px;color: gray"></i></a>
                                    <a href="{{ Route('deletedesignation', 'id=' . $designation->designation_id) }}"
