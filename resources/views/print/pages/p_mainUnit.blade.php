        ...
        <table class="table" id="myTable">
            <thead>
                <tr>
                    ...
                    <th onclick="sortTable(2)">Main Unit Remarks</th>
            ...
            <tbody>
                @foreach ($datas as $index => $item)
                    <tr>
                        ...
                        <td>{{ $item->main_unit_remarks }}</td>
                        <td>{{ $item->name }}</td>
                        <td>{{ date('d-M-Y', strtotime($item->main_unit_created_at)) }}</td>
                        @if (empty($type))
                            <td>
                                {{-- <div class="dropdown">
                                ...
                                <a href="{{ Route('mainUnitEdit', 'id=' . $item->main_unit_id) }}" class=""><i
                                        class="fa-solid fa-file-pen" style="font-size:16px;color: gray"></i></a>
                                <a href="{{ Route('mainUnitDelete', 'id=' . $item->main_unit_id) }}"
