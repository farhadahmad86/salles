        ...
        <table class="table" id="myTable">
            <thead>
                <tr>
                    ...
                    <th onclick="sortTable(4)">Remarks</th>
            ...
            <tbody>
                @foreach ($datas as $key => $sector)
                    <tr>
                        <td>{{ $key + $datas->firstItem() }}</td>
                        <td>{{ $sector->reg_name }}</td>
                        <td>{{ $sector->area_name }}</td>
                        <td>{{ $sector->sec_name }}</td>
                        <td>{{ $sector->sec_remarks }}</td>
                        <td>{{ $sector->name }}</td>
                        <td>{{ date('d-M-Y', strtotime($sector->sec_created_at)) }}</td>
                        @if (empty($type))
                            <td>
                                {{-- <div class="dropdown">
                                ...
                                <a href="{{ Route('editSector', 'id=' . $sector->sector_id) }}" class=""><i
                                        class="fa-solid fa-file-pen" style="font-size:16px;color: gray"></i></a>
                                <a href="{{ Route('deleteSector', 'id=' . $sector->sector_id) }}"
